...
@app.route("/checkinterestregistered/<int:id>", methods=["POST"])
@login_required
def interestregistered(id):
    try:
        interest = Interest.query.filter_by(user_id=current_user.id, project_id=id).first()
        if interest:
            response = {"message": "interest is registered"}
            return jsonify(response), 200
        else:
            response = {"message": "interest is not registered"}
            return jsonify(response), 404
    except Exception as e:
